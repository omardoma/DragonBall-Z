...
public class Game implements Serializable, PlayerListener, WorldListener, BattleListener
{
	...
	public void setBackground(Icon background)
	{
	...
	public void loadAttacks(String filePath) throws MissingFieldException, UnknownAttackTypeException
	{
		...
		for (int i = 0; i < lines.length; i++)
		{
			...
			if (lines[i].length < 3)
				throw new MissingFieldException(
						"File: " + filePath + ", Line: " + (i + 1) + ", Expected: 3, Missing: " + (3 - lines[i].length),
						filePath, i + 1, 3 - lines[i].length);
			...
			if (attackType.equalsIgnoreCase("SA"))
			{
				...
				if (attackType.equalsIgnoreCase("UA"))
				{
					attack = new UltimateAttack(name, damage);
				}
				else
					if (attackType.equalsIgnoreCase("MC"))
					{
						attack = new MaximumCharge();
					}
					else
						if (attackType.equalsIgnoreCase("SS"))
						{
							attack = new SuperSaiyan();
						}
						else
						{
							// If the attack type given is not one of the four
							// defined types.
							throw new UnknownAttackTypeException("File: " + filePath + ", Line: " + (i + 1)
									+ ", Attack Type: " + attackType + " is undefined.", filePath, i + 1, attackType);
						}
	...
	public void loadFoes(String filePath) throws MissingFieldException
	{
		...
		for (int i = 0; i < lines.length; i += 3)
		{
			...
			if (lines[i].length < 8)
				throw new MissingFieldException(
						"File: " + filePath + ", Line: " + (i + 1) + ", Expected: 8, Missing: " + (8 - lines[i].length),
						filePath, i + 1, 8 - lines[i].length);
